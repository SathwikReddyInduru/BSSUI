...
const IMSIPrefixComboBox = ({ value = [], onChange }) => {
  ...
  const css = {
    wrapper: { display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px' },
    input: { width: '120px', height: '28px', padding: '2px 6px', border: '1px solid #aaa', fontSize: '13px', boxSizing: 'border-box' },
    arrowGroup: { display: 'flex', flexDirection: 'column', gap: '4px' },
    arrowBtn: (disabled) => ({
      width: '22px', height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center',
      cursor: disabled ? 'not-allowed' : 'pointer',
      background: disabled ? '#f0f0f0' : '#d4d4d4',
      border: '1px solid #999', borderRadius: '2px', fontSize: '10px',
      userSelect: 'none', opacity: disabled ? 0.45 : 1,
    }),
    listBox: { width: '120px', height: '72px', border: '1px solid #aaa', overflowY: 'auto', background: '#fff', padding: '2px', boxSizing: 'border-box' },
    listItem: (highlighted) => ({
      padding: '2px 4px', cursor: 'pointer',
      background: highlighted ? '#3168c4' : 'transparent',
      color: highlighted ? '#fff' : '#000',
      whiteSpace: 'nowrap', userSelect: 'none', fontSize: '13px',
    }),
  };
...
const NetworkManagementModify = () => {
  ...
  useEffect(() => {
    ...
    const hasLowBalance = !!(d.firstThreshold && parseFloat(d.firstThreshold) > 0) ||
                          !!(d.secondThreshold && parseFloat(d.secondThreshold) > 0);

    const hasVoip = !!(d.domainName && d.domainName.trim());
    const hasSimUpload = d.bssui === 'Y' || d.hlrui === 'Y' || d.hssui === 'Y' ||
                         d.msgui === 'Y' || d.pcrfui === 'Y';
    ...
    setFormData({
      ...
      personalEmergencyCallCount: d.personalEmergencyCallCount?.toString() || '',
...
const checkDomain = (val) => {
  ...
  const validateForm = () => {
    ...
    if (formData.voipEnabled) {
      ...
      if (domval1 === '' && 
          !(formData.domainIp1 || formData.domainIp2 || formData.domainIp3 || formData.domainIp4)) {
  ...
  const stateOptions = [
    ...
    ...states.map(st => ({ value: st.stateCode, label: st.stateDescription })), // FIXED: value = code (matches API)
  ];
        ...
        <form onSubmit={handleSubmit} className={styles.networkForm}>
          ...
          <div className="form-row three-column">
            <FormField
              ...
              label={
  <>
    <span style={{ color: 'red' }}>*</span> {getLabel('networkmanagementmodify.networkName')}
  </>
}
            ...
            <FormField
              ...
              label={<><span style={{ color: 'red' }}>*</span> {getLabel('networkmanagementmodify.networkCode')}</>}
              value={formData.networkCode}
              onChange={handleInputChange}
            />
           

           <FormField fieldName="countryDesc" label={getLabel('networkmanagementmodify.country')} value={formData.countryDesc}  className="readonly-field" />
          </div>

          <div className="form-row three-column">
            

           <FormField fieldName="stateDesc" label={getLabel('networkmanagementmodify.state')} value={formData.stateDesc}  className="readonly-field" />
     

            <FormField fieldName="city" label={getLabel('networkmanagementmodify.city')} value={formData.city} onChange={handleInputChange} />
            <FormField fieldName="networkAddress" label={getLabel('networkmanagementmodify.networkAddress')} value={formData.networkAddress} onChange={handleInputChange} />
          </div>

          <div style={{ display: 'flex', gap: '24px', width: '100%', alignItems: 'flex-start' }}>
            <div style={{ flex: '3', minWidth: 0 }}>
              <label className="form-label">
                <span style={{ color: 'red' }}>*</span> {getLabel('networkmanagementmodify.description')}
              </label>
              <textarea
                name="description"
            ...
            <div style={{ flex: '1', minWidth: 0 }}>
             ...
             <div className="form-group">
              <label className="form-label">
                <span style={{ color: 'red' }}>*</span> {getLabel('networkmanagementmodify.imsiPrefix')}
              </label>
              <IMSIPrefixComboBox
          ...
          <div className="form-row three-column">
            ...
            <FormField
              ...
              label={<><span style={{ color: 'red' }}>*</span> {getLabel('networkmanagementmodify.msisdnLength')}</>}
            ...
            />
             <FormField
              ...
              label={getLabel('networkmanagementmodify.maxOperatorFailedLoginAttempts')}
            ...
            <FormField
              ...
              label={getLabel('networkmanagementmodify.maxSubscriberLoginAttempts')}
          ...
          <div className="form-row three-column">
            ...
            <FormField
              ...
              onChange={handleInputChange}
            />
            <FormField fieldName="voucherPrefix" label={getLabel('networkmanagementmodify.voucherPrefix')} value={formData.voucherPrefix} onChange={handleInputChange} />
            <FormField fieldName="subscriberDefaultPin" label={getLabel('networkmanagementmodify.subscriberDefaultPin')} value={formData.subscriberDefaultPin} onChange={handleInputChange} />
          </div>
          ...
          <div className="form-row three-column">
            ...
            <FormField fieldName="camelNodeId" label={getLabel('networkmanagementmodify.camelNodeId')} value={formData.camelNodeId} onChange={handleInputChange} />
            <div className="form-group">
              <label>{getLabel('networkmanagementmodify.g2BalanceRetention')}</label>
          ...
          <div className="form-row three-column">
            <FormField
              ...
              label={getLabel('networkmanagementmodify.personalEmergencyCallCount')}
          ...
          <div className="form-row three-column">
            <FormField fieldName="vmsNumber" label={getLabel('networkmanagementmodify.vmsNumber')} value={formData.vmsNumber} onChange={handleInputChange} />
            <div className="form-group">
              <label>{getLabel('networkmanagementmodify.statusTransitFlag')}</label>
              <select name="statusTransitFlag" value={formData.statusTransitFlag} onChange={handleInputChange}>
                <option value="Yes">Yes</option>
                <option value="No">No</option>
              </select>
            </div>
            <FormField fieldName="vccMsisdnSeries" label={getLabel('networkmanagementmodify.vccMsisdnSeries')} value={formData.vccMsisdnSeries} className="readonly-field" />
          </div>
          ...
          {formData.simUploadEnabled && (
            <div style={{ margin: '16px 0 32px 24px', padding: '20px', backgroundColor: '#f9f9f9', borderRadius: '8px', borderLeft: '4px solid #4a90e2' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px 32px' }}>
                {[
                  { name: 'bssUiEnabled',  label: getLabel('networkmanagementmodify.bssUiEnabled')  },
                  { name: 'hlrUiEnabled',  label: getLabel('networkmanagementmodify.hlrUiEnabled')  },
                  { name: 'hssUiEnabled',  label: getLabel('networkmanagementmodify.hssUiEnabled')  },
                  { name: 'msgUiEnabled',  label: getLabel('networkmanagementmodify.msgUiEnabled')  },
                  { name: 'pcrfUiEnabled', label: getLabel('networkmanagementmodify.pcrfUiEnabled') },
                ].map(({ name, label }) => (
            ...
            <>
              <div className="form-row three-column">
                <FormField
                  ...
                  label={<span style={{  }}>{getLabel('networkmanagementmodify.domainName')}</span>}
                  value={formData.domainName}
                  onChange={handleInputChange}
                />
                <div className="form-group">
                  <label>{getLabel('networkmanagementmodify.domainIpAddresses')}</label>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    {['domainIp1', 'domainIp2', 'domainIp3', 'domainIp4'].map(ip => (
                      <input
                        key={ip}
                        name={ip}
                        maxLength={3}
                ...
                <FormField fieldName="lrrGroupId" label={getLabel('networkmanagementmodify.lrrGroupId')} value={formData.lrrGroupId} onChange={handleInputChange} />
              </div>

              <div className="form-row three-column">
                <FormField fieldName="sipGroupId" label={getLabel('networkmanagementmodify.sipGroupId')} value={formData.sipGroupId} onChange={handleInputChange} />
                <FormField
                  fieldName="ringToneAlertInfoUrl"
                  label={getLabel('networkmanagementmodify.ringToneAlertInfoUrl')}
                  value={formData.ringToneAlertInfoUrl}
                  onChange={handleInputChange}
                />
                <FormField
                  fieldName="ringBackToneAlertInfoUrl"
                  label={getLabel('networkmanagementmodify.ringBackToneAlertInfoUrl')}
          ...
          {formData.lowBalanceEnabled && (
            <div className="form-row three-column">
              <FormField
                ...
                label={<span style={{  }}>{getLabel('networkmanagementmodify.firstThreshold')} </span>}
              ...
              <FormField
                ...
                label={<span style={{  }}>{getLabel('networkmanagementmodify.secondThreshold')} </span>}
          ...
          {/* Submit / Cancel */}
          <div className="button-group" style={{ marginTop: '48px', display: 'flex', gap: '16px', justifyContent: 'flex-end' }}>
